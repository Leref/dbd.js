module.exports = async (d) => {
  ...
  if (!reaction)
    return d.error(`❌ No emoji provided in \`$userReacted${inside}\``);
  const channel = d.client.channels.cache.get(channelID);

  if (!channel)
    return d.error(`❌ Invalid channel ID in \`$userReacted${inside}\``);
  ...
  const emoji = reaction.includes("<")
    ? reaction.split(":")[2].split(">")[0]
    : reaction;
