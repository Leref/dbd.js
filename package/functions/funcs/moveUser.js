module.exports = async (d) => {
  ...
  if (!inside.splits.length)
    return d.error(`:x: Invalid usage in $moveUser${inside.total}`);
  ...
  if (!user)
    return d.error(`:x: Invalid userID in \`$moveUser${inside.total}\``);

  const state = d.message.guild.voiceStates.cache.get(user.id);

  if (!state || !state.channel)
    return d.error(
      `:x: User is not in any voice channel in \`$moveUser${inside.total}\``
    );
  ...
  try {
    ...
    return d.error(
      `:x: Failed to move user from a voice channel in \`$moveUser${inside.total}\``
    );
  }
