...
module.exports = async (d) => {
  ...
  if (!time)
    return d.error(`:x: Not enough fields in \`$reactionCollector${inside}\``);

  if (!ms(time))
    return d.error(
      `:x: Failed to parse '${time}' in \`$reactionCollector${inside}\``
    );

  const msg = await d.message.channel.messages
    .fetch(messageID)
    .catch((err) => {});

  if (!msg)
    return d.error(`:x: Invalid message ID in \`$reactionCollector${inside}\``);
