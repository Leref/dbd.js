...
module.exports = async (d) => {
  ...
  while (i < inside.splits.length) {
    ...
    if (!songFilters[type])
      return d.error(
        `:x: Invalid song filter \`${type}\` in \`$songFilter${inside.total}\``
      );
  ...
  if (!server.ffmpegArgs.filter) server.ffmpegArgs.filter = {};
  ...
  while (i < inside.splits.length) {
    ...
    const bool =
      !value.length || isNaN(value) || !eval(exp.replace(/\{value\}/g, value));
    ...
    while (f < filters.length) {
      ...
      if (!bool)
        server.ffmpegArgs.filter[name] = prop.replace(/\{value\}/g, value);
      else delete server.ffmpegArgs.filter[name];
  ...
  const duration = server.connection.dispatcher.streamTime + (server.seek || 0);
