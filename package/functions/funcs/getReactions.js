module.exports = async (d) => {
  ...
  if (!channel)
    return d.error(`❌ Invalid channel ID in \`$getReactions${inside}\``);

  const msg = await channel.messages.fetch(messageID).catch((err) => {});

  if (!msg)
    return d.error(`❌ Invalid message ID in \`$getReactions${inside}\``);

  const emoji = reaction.addBrackets().includes("<")
    ? reaction.addBrackets().split(":")[2].split(">")[0]
    : reaction;
  ...
  return {
    code: code.replaceLast(
      `$getReactions${inside}`,
      reactions
        ? reactions.users.cache.map((u) =>
            options === "mention" ? u.toString() : u[options.toLowerCase()]
          )
        : "none"
    ),
  };
