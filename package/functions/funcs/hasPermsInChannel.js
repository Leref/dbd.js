...
module.exports = async (d) => {
  ...
  if (!channel || !user)
    return d.error(
      `❌ Invalid channel or user ID in \`$hasPermsInChannel${inside}\``
    );

  const pms = perms.map((key) => permissions[key]);

  if (pms.includes(undefined))
    return d.error(`❌ Invalid permissions in \`$hasPermsInChannel${inside}\``);

  const upms = channel.permissionsFor(userID);

  return {
    code: code.replaceLast(
      `$hasPermsInChannel${inside}`,
      upms && pms.every((p) => upms.has(p))
    ),
  };
};
