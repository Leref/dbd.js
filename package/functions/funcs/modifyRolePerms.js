...
module.exports = async (d) => {
  ...
  if (!role)
    return d.error(`❌ Invalid role ID in \`$modifyRolePerms${inside}\``);
  ...
  for (const perm of pms) {
    ...
    if (!p)
      return d.error(
        `❌ Invalid permission '${perm}' in \`$modifyRolePerms${inside}\``
      );
  ...
  if (!np)
    return d.error(`❌ Failed to set role permissions for ${role.name}!`);
