module.exports = async (d) => {
  ...
  if (!channel)
    return d.error(`❌ Invalid channel ID in \`$removeContains${inside}\``);
  ...
  const m = await channel
    .bulkDelete(
      messages
        .filter(
          (m) =>
            Date.now() - m.createdTimestamp < require("ms")("14d") &&
            wordOrWords.some(
              (word) =>
                (m.embeds &&
                  require("../../handlers/resolveEmbed")(m.embeds).includes(
                    word.addBrackets()
                  )) ||
                (m.content &&
                  m.content.toLowerCase().includes(word.addBrackets()))
            )
        )
        .map((m) => m.id)
    )
