module.exports = async (d) => {
  ...
  const all = await d.client.db.all("main", {
    filter: (x) =>
      x.key.startsWith(`${variable}_`) && x.key.split("_").length === 2,
  });
  ...
  for (const data of all.sort(
    ...
    const user = await d.client.users
      .fetch(data.key.split("_")[1])
      .catch((err) => {});
    ...
    if (user) {
      ...
      if (text.includes("{execute:")) {
        ...
        if (!awaited)
          return d.error(
            `❌ Invalid awaited command '${ins}' in \`$globalUserLeaderboard${inside}\``
          );
