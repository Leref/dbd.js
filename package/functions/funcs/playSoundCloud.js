...
module.exports = async (d) => {
  ...
  if (!client_id)
    console.warn(
      "soundcloud client_id is empty in $playSoundCloud, your bot could get rate limited if none was added. \nIf you don't know how to get one, Please follow this Procedure" +
        `
--------------------------------------------------------------------------
...
--------------------------------------------------------------------------`
  ...
  let response = await fetch(
    "https://api.leref.ga/soundcloud?url=" + encodeURIComponent(url)
  );
  ...
  const info = {
    ...
    duration_left: (server) => {
      ...
      const time = Math.floor(
        ms - (server.connection.dispatcher.streamTime + (server.seek || 0))
      );
  ...
  return {
    code: code.replaceLast(
      `$playSoundCloud${inside}`,
      video.title.removeBrackets()
    ),
  };
};
