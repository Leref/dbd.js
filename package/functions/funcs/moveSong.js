module.exports = async (d) => {
  ...
  if (!inside.total.startsWith("["))
    return d.error(":x: Invalid usage in $moveSong");

  if (!server) return d.error(":x: Nothing is playing!");

  if (!(n > 1) || n > server.songs.length)
    return d.error(`:x: Invalid number in \`$moveSong${inside.total}\``);
