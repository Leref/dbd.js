...
module.exports = async (d) => {
  try {
    ...
    if (!channel)
      return d.error(
        `:x: Invalid channel ID in \`$modifyChannelPerms${inside}\``
      );

    if (!perms.every((perm) => Object.keys(keyPerms).includes(perm.slice(1))))
      return d.error(
        `:x: Invalid permission in \`$modifyChannelPerms${inside}\``
      );

    const allow = perms
      .filter((perm) => perm[0] === "+")
      .map((perm) => keyPerms[perm.slice(1)]);
    ...
    const chAllow = channel.permissionOverwrites.get(roleOrUser)
      ? channel.permissionOverwrites
          .get(roleOrUser)
          .allow.toArray()
          .filter((perm) => !deny.includes(perm))
      : [];

    const chDeny = channel.permissionOverwrites.get(roleOrUser)
      ? channel.permissionOverwrites
          .get(roleOrUser)
          .deny.toArray()
          .filter((perm) => !allow.includes(perm))
      : [];
    ...
    const ch = await channel
      .updateOverwrite(roleOrUser, permsObject)
      .catch((err) => err.message);

    if (typeof ch === "string") {
      return d.error(
        `:x: Could not modify channel permissions! in \`$modifyChannelPerms${inside}\`\nError: ${ch}`
      );
    }
