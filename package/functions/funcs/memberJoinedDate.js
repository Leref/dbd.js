...
module.exports = async (d) => {
  ...
  if (inside.inside) {
    const [userID, date = "date"] = inside.splits;

    const m = await d.message.guild.members.fetch(userID).catch((err) => {});

    if (!m)
      return d.error(`:x: Invalid user ID in \`$memberJoinedDate${inside}\``);
    ...
    if (date === "date") {
      ...
      opt = parse(
        Object.entries(ms(Date.now() - m.joinedTimestamp))
          .map((x, y) => {
    ...
    return {
      code: code.replaceLast(
        `$memberJoinedDate`,
        parse(
          Object.entries(ms(Date.now() - d.message.member.joinedTimestamp))
            .map((x, y) => {
              if (x[1] && y < 4) return `${x[1]}${x[0][0]}`;
              return undefined;
            })
