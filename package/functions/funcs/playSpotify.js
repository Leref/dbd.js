...
module.exports = async (d) => {
  ...
  const Handler = new SpotifySearchHandler(
    d.client,
    d.message.guild.id,
    list,
    (video) => {
      const obj = {
        ...
        duration: () =>
          video.duration.toString().split("seconds").join("Seconds"),
    ...
    (videos, song) => {
      if (useFilter) {
        return (
          videos.find((vid) =>
            song.artists[0].name.split(" ").every((n) => vid.title.includes(n))
          ) ||
          videos.find((vid) =>
            song.artists[0].name
              .split(" ")
              .every((n) => vid.author.name.includes(n))
          )
        );
      } else {
        return videos[0];
