module.exports = async (d) => {
  ...
  if (!inside.splits.length)
    return d.error(`:x: Invalid usage in $muteUser${inside.total}`);
  ...
  if (!user)
    return d.error(`:x: Invalid userID in \`$muteUser${inside.total}\``);

  const state = d.message.guild.voiceStates.cache.get(user.id);

  if (!state || !state.channel)
    return d.error(
      `:x: User is not in any voice channel in \`$muteUser${inside.total}\``
    );
