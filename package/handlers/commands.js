...
const CommandHandler = async (client, message, db) => {
  if (client.messageEventOptions.guildOnly && message.channel.type === "dm")
    return;
  ...
  const commands = cmds
    .filter((c) => c.name !== "$alwaysExecute" && c.nonPrefixed !== true)
    .filter(
      (c) =>
        (c.name || "").toLowerCase() === cmd ||
  ...
  for (const command of commands) {
    if (command.asynchronous === false)
      interpreter(client, message, args, command, undefined);
