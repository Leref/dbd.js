...
module.exports = async (client, data, pulse) => {
  for (const command of pulse
    ? client.timeout_pulse_commands.array()
    : client.timeout_commands.array()) {
    const channelID = command.channel
      ? command.channel.includes("$")
        ? (await interpreter(
            client,
            {},
            [],
    ...
    const d = {
      guild:
        ...
        (channel ? channel.guild : undefined),
      author: client.user,
