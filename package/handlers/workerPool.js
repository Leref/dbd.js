...
class WorkerPool extends EventEmitter {
  constructor(threads, options = {}, callback = null) {
    ...
    this._limit =
      threads ||
      Math.min(
        Math.ceil(require("os").cpus().length * 0.74),
        process.env.MAX_WORKER_THREADS || 4
      );
  ...
  async spawn() {
    if (this.length >= this._limit) return 0;

    const worker = new Worker(this._options.filename, this._options);
  ...
  getWorker() {
    ...
    if (len < 2) return filtered.length ? filtered[0] : this._workers[0];
