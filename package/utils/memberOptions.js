module.exports = {
  ...
  nick:
    "The nickname of this user before it was updated, if any;.nickname || ''",
  roles:
    "The role names of the user before it was updated;.roles.cache.filter(r => r.name !== '@everyone').map(r => r.name.deleteBrackets()).join(', ')",
  partial: "Whether the member structure is cached;.partial ? `true` : `false`",
  premiumStamp:
    "The timestamp this user started boosting the server, or 0 if they're not boosting;.premiumSinceTimestamp || `0`",
  joinedStamp:
    "The timestamp when this user joined the server;.joinedTimestamp",
  voiceID: "The ID of the voice channel this user's in;.voice.channelID || ''",
  displayHex:
    "Returns the hex color of this user's highest role;.displayHexColor",
  highestRoleID:
    "The ID of the highest role of this user in the guild;.roles.highest.id",
  permissions:
    "The permissions for this member;.permissions.toArray().goof('_')",
  ...
  displayColor:
    "Displays the hex color of the highest role in this user;.displayHexColor",
  status: "The status for this user;.presence.status",
  activities:
    "The activities for this user;.presence.activities.map(c => c.name).join(', ')",
